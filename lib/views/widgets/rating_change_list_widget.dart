...
class RatingChangeListWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocListener<ContestStandingsCubit, ContestStandingsState>(
      listener: (context, state) {
        if (state is ContestStandingsLoading) {
          showDialog(
            context: context,
            builder: (context) => const ContestStandingsDialog(),
          );
        }
      },
      child: BlocBuilder<RatingChangeListCubit, RatingChangeListState>(
        builder: (context, state) {
          if (state is RatingChangeListLoaded) {
                            ...
                            itemBuilder: (BuildContext context, int index) {
                              return Padding(
                                ...
                                child: GestureDetector(
                                  onTap: () {
                                    BlocProvider.of<ContestStandingsCubit>(
                                            context)
                                        .makeContestStandingsDialog(state
                                            .ratingChanges[index].contestId);
                                  },
                                  child: Container(
                                    decoration: BoxDecoration(
                                        border: Border(
                                            left: BorderSide(
                                      color: (state.ratingChanges[index]
                                                      .newRating -
            ...
            dev.log("Error: (Rating Change List widget) - ${state.message}");
